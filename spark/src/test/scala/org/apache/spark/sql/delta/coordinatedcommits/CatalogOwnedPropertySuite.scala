...
class CatalogOwnedPropertySuite extends QueryTest
  ...
  test("[CREATE] Specifying table UUID when creating catalog-owned " +
      " table should be blocked") {
    val error = intercept[DeltaUnsupportedOperationException] {
      withTable("t1") {
        sql(s"CREATE TABLE t1 (id LONG) USING delta TBLPROPERTIES " +
          s"('delta.feature.${CatalogOwnedTableFeature.name}' = 'supported', " +
  ...
  test("[CTAS] Specifying table UUID when creating catalog-owned " +
      " table via CTAS should be blocked") {
    withTable("t1", "t2") {
      ...
      val error = intercept[DeltaUnsupportedOperationException] {
        sql(s"CREATE TABLE t2 USING delta TBLPROPERTIES " +
          s"('delta.feature.${CatalogOwnedTableFeature.name}' = 'supported', " +
          s"'${UCCommitCoordinatorClient.UC_TABLE_ID_KEY}' = '${UUID.randomUUID().toString}') " +
          s"AS SELECT * FROM t1")
      }
      ...
      // Normal CTAS should not be blocked
      sql(s"CREATE TABLE t2 USING delta TBLPROPERTIES " +
        s"('delta.feature.${CatalogOwnedTableFeature.name}' = 'supported') " +
        s"AS SELECT * FROM t1")
  ...
  test("[CREATE LIKE] Table UUID & CatalogOwnedTableFeature should not be copied from " +
      "source table to target table") {
    withTable("t1", "t2", "t3", "t4", "t5", "t6") {
      // Source tables
      createTableAndValidateCatalogOwned(tableName = "t1", withCatalogOwned = true)
      val ucTableIdT1 = getUCTableIdFromTable(tableName = "t1")
      createTableAndValidateCatalogOwned(tableName = "t2", withCatalogOwned = false)

      // From catalog-owned source table
      sql("CREATE TABLE t3 LIKE t1")
      validateCatalogOwnedAndUCTableId(tableName = "t3", expected = false)

      sql("CREATE TABLE t4 LIKE t1 TBLPROPERTIES " +
        s"('delta.feature.${CatalogOwnedTableFeature.name}' = 'supported')")
      val ucTableIdT4 = getUCTableIdFromTable(tableName = "t4")
      validateCatalogOwnedAndUCTableId(tableName = "t4", expected = true)
      assert(ucTableIdT1 !== ucTableIdT4)

      // From normal delta source table
      sql("CREATE TABLE t5 LIKE t2")
      validateCatalogOwnedAndUCTableId(tableName = "t5", expected = false)

      sql("CREATE TABLE t6 LIKE t2 TBLPROPERTIES " +
        s"('delta.feature.${CatalogOwnedTableFeature.name}' = 'supported')")
      validateCatalogOwnedAndUCTableId(tableName = "t4", expected = true)
    }
  }
  ...
  test("[CREATE LIKE] CatalogOwned should be enabled with default spark session " +
     "configuration") {
    withSQLConf(
        TableFeatureProtocolUtils.defaultPropertyKey(CatalogOwnedTableFeature) -> "supported") {
      withTable("t1", "t2") {
        withoutDefaultCCTableFeature {
          sql("CREATE TABLE t1 (id LONG) USING delta CLUSTER BY (id)")
        }
        validateCatalogOwnedAndUCTableId(tableName = "t1", expected = false)
        // `t1` should have [[ClusteringTableFeature]] enabled
        val (_, snapshot1) = DeltaLog.forTableWithSnapshot(spark, new TableIdentifier("t1"))
        val readerWriterFeatureNames1 =
          snapshot1.protocol.readerAndWriterFeatureNames
        assert(readerWriterFeatureNames1.contains(ClusteringTableFeature.name))

        withSQLConf(TableFeatureProtocolUtils
            .defaultPropertyKey(IcebergCompatV2TableFeature) -> "supported") {
          sql("CREATE TABLE t2 LIKE t1")
        }
        // `t2` will *not* have [[IcebergCompatV2TableFeature]] enabled by default,
        // because the source table protocol will override the generated protocol
        // by default spark configuration.
        // However, for [[CatalogOwnedTableFeature]], we choose to enable it
        // if being set in the default spark configuration.
        // This is different from the normal behavior/semantic of CREATE LIKE.
        validateCatalogOwnedAndUCTableId(tableName = "t2", expected = true)
        val (_, snapshot2) = DeltaLog.forTableWithSnapshot(spark, new TableIdentifier("t2"))
        assert(!snapshot2.protocol.readerAndWriterFeatureNames.contains(
          IcebergCompatV2TableFeature.name))
      }
    }
  }
  test("Existing table features in the protocol of normal delta table should be preserved " +
      "after REPLACE w/ default CC enablement") {
    withTable("t1", "source") {
      // Source table.
      sql("CREATE TABLE source (id LONG) USING delta")
      sql("INSERT INTO source VALUES (3), (4), (5)")

      // Create a normal delta table w/ DV and ICT enabled.
      sql("CREATE TABLE t1 (id LONG) USING delta")
      sql("INSERT INTO t1 VALUES (1), (2)")
      sql("ALTER TABLE t1 SET TBLPROPERTIES " +
        s"('${DeltaConfigs.IN_COMMIT_TIMESTAMPS_ENABLED.key}' = 'true')")
      validateInCommitTimestampPresent(
        tableName = "t1",
        expected = true,
      ...
      withDefaultCCTableFeature {
        sql("REPLACE TABLE t1 USING delta AS SELECT * FROM source")
        validateCatalogOwnedAndUCTableId(tableName = "t1", expected = false)
        validateProtocolMinReaderWriterVersion(
          tableName = "t1",
          expectedMinReaderVersion = 3,
          expectedMinWriterVersion = 7)
        val (_, snapshot) = DeltaLog.forTableWithSnapshot(spark, TableIdentifier("t1"))
        // Table features should not be dropped during REPLACE.
        Seq(DeletionVectorsTableFeature, InCommitTimestampTableFeature).foreach { feature =>
          assert(snapshot.protocol.readerAndWriterFeatureNames.contains(feature.name))
        }
        checkAnswer(sql("SELECT * FROM t1"), Seq(Row(3), Row(4), Row(5)))
      }
    }
  }
