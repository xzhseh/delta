...
class CatalogOwnedEnablementSuite
  ...
  test("Testing usage log for commit coordinator population w/o catalog table w/o ucTableId") {
      ...
      val usageLog = Log4jUsageLogger.track {
        ...
        log.metric == "tahoeEvent" &&
          log.tags.getOrElse("opType", null) ==
            CatalogOwnedUsageLogs.COMMIT_COORDINATOR_POPULATION_WITHOUT_UC_TABLE_ID
      }

      assert(usageLog.nonEmpty, "Should have usage log for WITHOUT_UC_TABLE_ID scenario")
