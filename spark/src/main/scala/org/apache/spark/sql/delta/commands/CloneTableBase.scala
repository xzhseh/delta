  ...
  private def determineTargetProtocol(
    ...
    val configWithOverrides = txn.metadata.configuration ++ validatedConfigurations ++
      (if (CatalogOwnedTableUtils.defaultCatalogOwnedEnabled(spark) &&
        ...
        // Append [[CatalogOwnedTableFeature]] to the `configWithOverrides`
        // to ensure the final target protocol contains CatalogOwned if enabled by default.
        // We need this because CatalogOwned is enabled through single protocol without
        // auxiliary metadata.
