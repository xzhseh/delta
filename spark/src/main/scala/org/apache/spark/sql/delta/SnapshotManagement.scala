      ...
      if ((deltaVersions.head to deltaVersions.last) != deltaVersions) {
        recordDeltaEvent(
          ...
          data = Map(
            ...
            // [[(..more..).SnapshotManagementEdge.verifyLogSegmentVersions.verifyDeltaVersions]].
            //   ^ -------------------- The Caller  ----------------------^ ^ ---- Current ---^
            ...
            "unsafeVolatileSnapshot.latestCheckpointVersion" ->
              cachedSnapshot.map(_.checkpointProvider.version).getOrElse(-1L),
            "unsafeVolatileSnapshot.latestSnapshotVersion" ->
              cachedSnapshot.map(_.version).getOrElse(-1L),
            "unsafeVolatileSnapshot.checksumOpt" ->
              cachedSnapshot.map(_.checksumOpt).orNull
          ))
